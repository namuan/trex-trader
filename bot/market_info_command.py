    ...
    chart_handler = ConversationHandler(
        ...
        states={
            WorkflowEnum.MARKET_INFO:
                [RegexHandler("^(" + regex_coin(market_summary_alt_coins()) + ")$", send_market_info_msg,
                              pass_chat_data=True),
...
def send_market_info_msg(bot, update, chat_data):
    ...
    update.message.reply_text(reply_msg, reply_markup=keyboard_cmds(), parse_mode=ParseMode.MARKDOWN)
    return ConversationHandler.END


def format_price_data(symbol):
    data = bittrex.get_price(symbol)
    return "Last {:06.8f}".format(
        data.get('Last')
    )
