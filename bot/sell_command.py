    ...
    chart_handler = ConversationHandler(
        ...
        states={
            WorkflowEnum.TRADE_CALCULATE_SELL_COST:
                [
                    RegexHandler("^(" + regex_coin(market_summary_alt_coins()) + ")$", calculate_sell_order_size,
                                 pass_chat_data=True),
                    RegexHandler("^(CANCEL)$", cancel)
                ],
            WorkflowEnum.TRADE_CHANGE_SELL_PRICE:
                [
                    RegexHandler("^\+(\d+)\%$", change_sell_price, pass_chat_data=True, pass_groups=True),
                    RegexHandler("^([\d\.]+)\s+([\d\.]+)$", change_sell_price, pass_chat_data=True,
                                 pass_groups=True),
                    RegexHandler("^(SELL)$", place_sell_order, pass_chat_data=True),
                    RegexHandler("^(CANCEL)$", cancel)
                ]
        },
...
def sell_cmd(bot, update, chat_data):
    ...
    alt_wallets = [w.get('currency') for w in bittrex.get_balances() if w.get('currency') != 'BTC']
    ...
    reply_mrk = ReplyKeyboardMarkup(
        build_menu(coin_buttons_from_list(alt_wallets), n_cols=3, footer_buttons=cancel_btn), resize_keyboard=True)
...
def calculate_sell_order_size(bot, update, chat_data):
    ...
    if not available_in_balance:
        return cancel(bot, update, "You do not have any coins available for {} in your wallet".format(currency))

    _, commission, total = bittrex.trade_commission(available_in_balance, bid_price)

    reply_msg = "{} bid is *{:06.8f}*\n" \
                "order size is *{:06.8f}*.\n" \
...
def get_all_from_balance(currency):
    ...
    return next((c.get('available') for c in bittrex_balances if c.get('currency') == currency), None)
